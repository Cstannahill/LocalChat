...
public sealed class OpenRouterInferenceProvider
{
    private readonly HttpClient _httpClient;
    private readonly OpenRouterOptions _options;
    private readonly ILogger<OpenRouterInferenceProvider> _logger;

    public OpenRouterInferenceProvider(
        HttpClient httpClient,
        OpenRouterOptions options,
        ILogger<OpenRouterInferenceProvider> logger)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("No OpenRouter model was provided.");
        }
        ...
        {
            request.Headers.TryAddWithoutValidation("X-OpenRouter-Title", _options.AppTitle);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new InvalidOperationException(
                $"OpenRouter request failed with status {(int)response.StatusCode}: {errorBody}");
        }
        ...
        while (!reader.EndOfStream)
        {
            ...
            {
                _logger.LogWarning(ex, "Failed to parse OpenRouter streaming chunk: {Chunk}", payload);
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(
                $"OpenRouter response parsing failed. Response body: {body}",
                ex);
        }
    }
}
